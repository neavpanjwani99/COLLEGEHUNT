      ...
      {(["placement", "fees", "location"] as const).map((key) => (
        <div key={key}>
          <div className="mb-2 flex items-center justify-between text-sm">
            ...
            <span className="font-semibold text-[#FF385C]">{weights[key]}%</span>
          </div>
          <input type="range" min="0" max="100" step="5" value={weights[key]} onChange={(event) => update(key, Number(event.target.value))} className="w-full h-1.5 bg-[#E5E7EB] rounded-lg appearance-none cursor-pointer accent-[#FF385C]" />
        </div>
      ))}
      <div className="text-sm font-medium text-[#111827]">Total: 100%</div>
      <button type="button" className="text-sm font-semibold text-[#FF385C] hover:underline" onClick={() => setWeights({ placement: 60, fees: 30, location: 10 })}>
        Reset weights
      </button>
    </div>
  );
